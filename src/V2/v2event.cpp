...
{
    instance()->addHandlerImpl(eventType, &handler);
}
...
void V2EventSystem::addHandlerImpl(V2Event::Type eventType, V2EventHandler &handler)
{
...
void V2EventSystem::sendEventInternal(V2Event *event)
{
    ...
    foreach(V2EventHandler* handler, handlers) {
        switch(event->type()){
            ...
            case V2WindowResizeEventType:    handler->v2resizeEvent((const V2WindowResizeEvent*)event);   break;
